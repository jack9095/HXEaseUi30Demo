    ...
    public void onSetUpView() {
        ...
        if(emojicon != null){
            if(emojicon.getBigIcon() != 0){
                ImageLoaderManager.getInstance().displayImageNetUrl(activity,emojicon.getBigIcon()+"",R.drawable.ease_default_expression,imageView);
//                Glide.with(activity).load(emojicon.getBigIcon()).placeholder(R.drawable.ease_default_expression).into(imageView);
            }else if(emojicon.getBigIconPath() != null){
                ImageLoaderManager.getInstance().displayImageNetUrl(activity,emojicon.getBigIconPath()+"",R.drawable.ease_default_expression,imageView);
