...
public class DemoDBManager {
    ...
    private DemoDBManager(){
        dbHelper = DbOpenHelper.getInstance(HXApplication.getInstance().getApplicationContext());
    }
...
//     * @param contactList
    ...
    synchronized public void saveContactList(List<EaseUser> contactList) {
        ...
        FLogUtil.e("数据库存入通讯录的数据", FGsonUtils.toJson(contactList));
        if (db.isOpen()) {
...
//                db.replace(UserDao.TABLE_NAME, null, values);
                ...
                FLogUtil.d("插入数据结果 = ",insert);
            }
        }
    }

    // TODO 开启事务批量插入，使用SQLiteStatement  速度最快
//    synchronized public boolean saveContactList(List<EaseUser> list) {
//        if (null == dbHelper || null == list || list.size() <= 0) {
//            return false;
//        }
//        SQLiteDatabase db = null;
//        try {
//            db = dbHelper.getWritableDatabase();
//            String sql = "insert into " + UserDao.TABLE_NAME + "("
...
//                stat.bindString(1, user.getUsername());
//                stat.bindString(2, user.getNickname());
//                stat.bindString(3, user.getAvatar());
...
//    }
     ...
     * @return
     */
//    public Map<String, EaseUser> getContactListFFF() {
    synchronized public Map<String, EaseUser> getContactListFFF() {
        ...
        FLogUtil.d("数据库取出通讯录的数据db ", FGsonUtils.toJson(users));
        return users;
    }
