...
public class ContactItemView extends LinearLayout{
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        ImageView avatar = (ImageView) findViewById(R.id.avatar); // 群聊头像
        unreadMsgView = (TextView) findViewById(R.id.unread_msg_number);
        TextView nameView = (TextView) findViewById(R.id.name);
        if(image != null){
            avatar.setImageDrawable(image);
            ImageLoaderManager.getInstance().displayCircleImage(
                    getContext(),R.drawable.em_groups_icon,avatar,R.drawable.em_groups_icon
            );
        } else {
            ImageLoaderManager.getInstance().displayCircleImage(
                    getContext(),R.drawable.em_groups_icon,avatar,R.drawable.em_groups_icon
