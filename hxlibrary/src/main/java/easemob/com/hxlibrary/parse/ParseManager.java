package easemob.com.hxlibrary.parse;
...
import easemob.com.hxlibrary.app.DemoHelper.DataSyncListener;
		...
		pQuery.getFirstInBackground(new GetCallback<ParseObject>() {
			...
			public void done(ParseObject pUser, ParseException e) {
				if(pUser!=null){
					...
					if(callback!=null){
					    EaseUser user = DemoHelper.getInstance().getContactListWWW().get(username);
						if(user!=null){
